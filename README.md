...
| 阶段 4 回归+切换 | ✅ **生产切换完成（2026-09-26 步骤 020，0.29 栈服务运行中）** | 切换演练 ✅ + 四件验证/日志三行 ✅（pool 114,974 容量无缩水、xqa+writer+spec capture 在）；回退=预案 §4 两步（0.27 冻结锚）；**⭐待优化项头名：KV offload×0.29 Windows 崩溃修复→回填**（用户多轮对话场景依赖 offload 做前缀缓存 CPU 延伸；现状 prefix-cache 工作、容量不受影响）；vision 面未切换（A7 锚挂账） |
