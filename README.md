...
| 阶段 4 回归+切换 | 🔶 **切换面就绪（2026-09-26 步骤 019，待择时执行切换）** | 生产 launcher 换血 ✅（`tools/serve_gsq_prod029_n2.cmd` + 模型目录入口）、回退预案 ✅（`docs/切换与回退预案.md`）、切换演练 ✅（冒烟全绿+无溢出）；**KV offload×0.29 Windows 崩溃挂账**（生产配方定格无 offload，容量/锚不受影响）；vision 面未切换（A7 锚挂账） |
