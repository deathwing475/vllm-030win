...
| T-perf 诊断战役 | ✅ **诊断收官 + 优化首胜（2026-09-26 步骤 022/023）** | 诊断：溢出排除、GPU 贴权重读地板 14.9ms/步、其余=图外 CPU 派发；**boot 方差根因修复（底座 `3498ef1`：AOT 缓存尾换行非对称→每 boot 重编译→生成码漂移，修复后同产物 ±0.4%）**；优化首胜：**草稿 FULL_DECODE_ONLY 图化解耦（`dflash/speculator.py` 草稿图模式原被绑死在 target 模式）⇒ decode 31.53→27.35ms（−13.3%）、吞吐 +15.5%、正确性/接受率保持**（实验 B GDN 入图=阴性已回退）。挂账：GDN 包装减脂、attention/kv_update 包装、快慢态产物差 |
