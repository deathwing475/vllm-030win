...
| T-perf 诊断战役 | ✅ **首战双结（2026-09-26 步骤 022）** | 溢出排除（Shared 488MB 平线+行为签名阴性）；decode 步内剖：GPU 贴权重读地板 14.9ms/步、每步其余=图外 CPU 派发（spec 理论 2.5× 实拿 1.27×）；**⭐boot 级性能方差根因修复（底座 `3498ef1`：AOT 缓存校验尾换行非对称→每 boot 重编译→生成码漂移；修复后同产物两 boot ±0.4%）**；历史 O(L)/平步长异常重注=生成码方差。挂账：CPU 派发优化战役（估 30.7→~20ms） |
