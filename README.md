...
| 阶段 2 自研迁移 | ✅ **五批全收官（含批 3b）** | 批1 协议/GC/hybrid（`57188ef`）、批2 TQ+KV（`f56c964`，KV 域零搬运）、批3 量化域（`7d4c0e1`）、批3b PIECEWISE 恢复（`5fe078a`）、批4 投机解码域（`76bec8b`，9/17 零搬运）、**批5 nvfp4/SM120（`78db8a6`，A1 同口径 PPL 8/8 全绿 + flashinfer 0.6.18 升级）**；剩三线合并回归（spec 端到端 + PIECEWISE + 无 spec 吞吐） |
