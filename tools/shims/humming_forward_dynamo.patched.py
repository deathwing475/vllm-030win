...
# ZCode shim #9 (batch-3, 2026-09-26): humming_forward does Python-level
# argument massaging (json.loads on compute_config etc.) that torch.compile's
# dynamo cannot trace -- during cudagraph capture re-tracing it receives a
# non-str compute_config and json.loads explodes, killing the engine at
# startup (PIECEWISE and FULL_AND_PIECEWISE alike). Disable tracing for this
# wrapper so dynamo treats it as an opaque eager call; numeric behavior is
# identical to the eager path (verified by the PPL anchor). Apply to
# humming/forward.py in site-packages whenever humming-kernels is
# (re)installed -- see SHIMS.md.
try:
    humming_forward = torch._dynamo.disable(humming_forward, recursive=True)
except Exception:  # pragma: no cover - older torch without dynamo
    pass
