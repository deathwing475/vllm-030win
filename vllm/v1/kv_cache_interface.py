...
class KVCacheSpec:
    ...
    def prefix_cacheable(self) -> bool:
        ...
        Consumed by the block-size resolution and the KV cache coordinator:
        a group that cannot serve fine-grained (hash-block-aligned) lookups --
        e.g. a sliding-window draft -- must not drag the cache-hit alignment
        down to the hash block size.
        """
        return True
...
class SlidingWindowSpec(AttentionSpec):
    ...
    def real_page_size_bytes(self) -> int:
        ...
        if self.kv_quant_mode == KVQuantMode.INT4_PER_TOKEN_HEAD:
            # INT4 packs two values per byte, so the stored head dim is half
            # the logical one. Dropping this halves the page a sliding-window
            # layer claims, and the reshape then reads a page twice the size
            # of the buffer it was allocated.
            last_dim = self.head_size // 2 + self.head_size_v // 2
        else:
            last_dim = self.head_size + self.head_size_v
        return (
        ...
        if self.page_size_padded is not None:
            if self.page_size_padded < page_size:
                ...
                if os.getenv("VLLM_EXPERIMENTAL_ALLOW_SMALL_HYBRID_BLOCK", "0") != "1":
                    raise ValueError("mamba page_size_padded is smaller than state size")
                return page_size
            return self.page_size_padded
        return page_size
