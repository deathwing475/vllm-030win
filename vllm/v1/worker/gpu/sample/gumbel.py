...
def gumbel_noised_argmax(
    ...
    what lets a draft and its verification agree. The walk over a candidate
    shortlist needs exactly that: it must score ids the same way the full-vocab
    sampler would.

    The noise is drawn from this tree's Philox primitives. That differs from the
    drop-in kernel's murmur3 stream, so a `draft_sample_method='probabilistic'`
    draft and its verifier must be revalidated together before that path is
    trusted; at the default `'greedy'` the temperature is zero and every draw
    here is inert.
    """
