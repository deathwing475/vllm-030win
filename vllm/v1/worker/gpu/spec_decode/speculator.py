...
class DraftModelSpeculator(BaseSpeculator):
    def __init__(self, vllm_config: VllmConfig, device: torch.device):
        ...
        if self.speculative_config.draft_sample_method == "probabilistic":
            ...
            logits_dtype, logits_fill = self.draft_logits_spec(vllm_config)
        ...
        # Adaptive verification is an upstream feature this fork does not carry:
        # the acceptance estimator it needs is not ported, so the flag stays off
        # and `_maybe_predict_acceptance` is a no-op.
        self.enable_adaptive_verification = False

    def draft_logits_spec(self, vllm_config: VllmConfig) -> tuple[torch.dtype, float]:
        """Dtype and fill for the cached proposal distribution.

        Speculators that write only a subset of columns each step must leave the
        rest in a state the verifier can read, so they override this.
        """
    ...
    def _maybe_predict_acceptance(
        ...
        """No-op: this fork does not carry the online acceptance estimator."""
