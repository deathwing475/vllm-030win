...
def resolve_kv_cache_block_sizes(
    ...
    # scheduler block size. Read the mode from the resolved group spec, not by
    # comparing its block size against ``cache_config.block_size``: that field
    # is rewritten to the smallest group block size once the KV config is built,
    # so a heterogeneous pool (e.g. a skip-quant draft layer alongside a
    # quantized target) makes the comparison misfire and sends us down the
    # back-off branch with hash == LCM, which then fails the connector's
    # divisibility assert.
    if any(
    ...
    for layer_name, layer_spec in kv_cache_spec.items():
        if layer_spec.page_size_bytes == max_page_size:
            ...
            if max_page_size % layer_page_size == 0:
                ...
                # Pad the page up to the shared size and grow the block to use
                # it. Padding while keeping a tiny block bills a whole shared
                # page per `block_size` tokens, over-booking the pool by
                # page / (block * per_token).
                #
                # The raised block must divide the primary block size, whose
                # page defined `max_page_size`: prefix caching uses the LCM of
                # the groups' block sizes as its hash granularity, and a block
                # coprime with the primary's inflates that LCM (3104 vs 3008
                # gives 291776) so the divisibility assert fails.
                per_token = max(layer_page_size // max(layer_spec.block_size, 1), 1)
                # Divisors of the primary block keep the LCM at the primary
                # block size; a coprime block would inflate it.
                grid = primary_block_size
                fits = [
                    d
                    for d in range(layer_spec.block_size, grid + 1)
                    if d % layer_spec.block_size == 0
                    and grid % d == 0
                    and d * per_token <= max_page_size
                ]
                new_block_size = max(fits) if fits else layer_spec.block_size
                new_spec = replace(
                    layer_spec,
