...
class HybridKVCacheCoordinator(KVCacheCoordinator):
    ...
    def __init__(
        ...
        if self.enable_partial_hash_hits:
            # Fine-grained hits hand every manager an alignment of
            # hash_block_size, which managers that need block-aligned lookups
            # cannot honour: their own block size has to divide the alignment.
            # Turn the feature off when any prefix-cacheable manager declines.
            # Without this a sliding-window draft group (e.g. DFlash2's)
            # alongside an align-mode Mamba target dies in
            # find_longest_cache_hit with "SlidingWindowManager does not
            # support fine-grained (partial) cache hits".
            unsupported = {
