...
class FlashInferMetadataBuilder(AttentionMetadataBuilder[FlashInferMetadata]):
    ...
    def __init__(
        ...
        if self.kv_cache_spec.kv_quant_mode != KVQuantMode.NONE:
            ...
            if self.is_kvcache_nvfp4:
                if current_platform.is_device_capability_family(120):
                    ...
                    self.kv_cache_dtype = FlashInferBackend.get_dtype_for_flashinfer("nvfp4")
                else:
                    if (
                        ...
                        raise ValueError(
                            ...
                            "FlashInfer path or consumer Blackwell (SM120)."
                        )
        ...
        # NOTE (E6.1, 2026-09-12): upstream vLLM #46329 patch 09 routes sm12x
        # nvfp4 decode to the FI-native FA2 wrapper instead of XQA, on the claim
        # that "XQA measured 0.7-1.2% slower at equal cudagraph mode (#49818)".
        # Ported and measured here on one binary (toggle-gated A/B, decode @100k,
        # salt 51, 2 runs/arm): XQA 55.43/55.35 vs FA2 54.84/54.74 t/s => FA2 is
        # 1.08% *slower* on this hardware (within-arm spread 0.1%). Both arms were
        # correct (needle 8/8) and zero-spill. So we deliberately deviate from
        # upstream and keep XQA. See
        # research/experiments/e6-upstream-solutions/analysis.md (E6.1).
        ...
        import os as _os
        logger.info_once(
            "HUMMING-DEBUG init kernel=%r pid=%s file=%s",
            self.flashinfer_trtllm_api_decode_kernel, _os.getpid(), __file__,
        )
        decode_backend = (
    ...
    def get_cudagraph_support(
        ...
        # NOTE (E6.1): upstream #46329 patch 09 additionally advertises
        # UNIFORM_SINGLE_TOKEN_DECODE for sm12x + nvfp4 (because its FA2 decode
        # route cannot honour uniform-batch capture). We measured that route to
        # be 1.08% slower than our XQA route and reverted it, so UNIFORM_BATCH
        # stays correct here -- verified by 8/8 needle recall under captured
        # FULL graphs at cudagraph sizes [1, 2].
    ...
    def _get_flashinfer_trtllm_api_decode_kernel() -> FlashInferDecodeKernel:
        import os as _os
        logger.info_once("HUMMING-DEBUG chooser called, pid=%s", _os.getpid())
        if current_platform.is_device_capability(90):
            return FlashInferDecodeKernel.XQA
        if current_platform.is_device_capability_family(120):
            # XQA is the *official* SM12x NVFP4-KV decode kernel: flashinfer's
            # own auto-selector picks it here (decode.py: "For sm_90 ... and
            # sm_120/sm_121 ..., auto will choose xqa backend"), and xqa.py
            # asserts "XQA NVFP4 KV is only supported on SM12x GPUs".
            #
            # Correction (E5, 2026-09-12): an earlier version of this comment
            # blamed trtllm-gen for producing garbage on SM120.  That is
            # impossible -- trtllm-gen FMHA cannot launch on SM120 at all
            # (fmhaRunner.cuh hard-checks kSM_100||kSM_103||kSM_107 and raises
            # "Unsupported architecture"; the guard is present in every tag
            # from v0.5.0 through v0.7.0rc1), because tcgen05 is absent from
            # the sm120 ISA.  The garbage actually came from the decode route
            # that ran before this branch existed, the flashinfer-native FA2
            # reader (see findings.md); XQA + carved NVFP4 views fixed it.
            # Details: research/experiments/e5-nvfp4-kernel-landscape/analysis.md
            logger.info_once("HUMMING-DEBUG chooser -> XQA (SM120)")
            return FlashInferDecodeKernel.XQA
...
class FlashInferImpl(AttentionImpl):
    ...
    def forward(
        ...
        if attn_metadata.use_cascade and not self.is_kvcache_nvfp4:
            ...
            # (same failure class as the docker XQA decode wiring bug), so
            # nvfp4 always takes the standard path below.
    ...
    def do_kv_cache_update(
        ...
        if self.kv_sharing_target_layer_name is None:
            ...
            if self.is_kvcache_nvfp4 and self.use_fa2_nvfp4_kv:
                # E6.2 (adopted 2026-09-12): FlashInfer's fused CUDA writer, one
                # kernel per layer instead of the ~40 torch ops of the reference
                # writer it replaces.  In-situ A/B on the delivered launch
                # script, decode @100k: 60.03 vs 55.28 t/s (+8.6%), prefill
                # unchanged, needle sweep 8/8, vision and spill gates green.
