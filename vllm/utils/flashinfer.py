...
def supports_trtllm_attention(is_prefill: bool = False) -> bool:
    ...
    # Windows research port (research/native-win): SM12x also has XQA decode
    # only (docker 0.29 semantics) - required for NVFP4 KV on SM120.
    if current_platform.is_device_capability(90) or current_platform.is_device_capability_family(
        120
    ):
