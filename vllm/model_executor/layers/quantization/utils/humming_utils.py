...
def prepare_humming_linear_layer_config(
    ...
    """Convert a linear layer's checkpoints tensors into humming's layout and
    return the LayerConfig the kernel needs at call time.

    Mirrors vLLM 0.30: the layer config is built with `prepare_layer_config`
    and the tensors are rewritten with `transform_humming_tensors`. The config
    is returned rather than stashed on the layer, because the humming forward
    entry point takes it as an argument.
    """
    from vllm.utils.humming import (
        BaseInputSchema,
        BaseWeightSchema,
