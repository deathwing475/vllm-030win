...
class INCConfig(QuantizationConfig):
    ...
    def get_quant_method(self, layer: torch.nn.Module, prefix: str):
        ...
        # Explicit "format": "fp8" layers use per-tensor fp8 weight-only
        # storage -- the kernel_projection fp8 width experiment. The checkpoint
        # stores `<name>.weight` (float8_e4m3fn, [out, in]) + `<name>.weight_scale`.
        # vLLM's Fp8LinearMethod would be the natural fit but its cutlass
        # scaled_mm path crashes on this sm120 Windows build, so INCFp8Linear
        # dequantises and runs a plain GEMM instead (see its docstring).
        raw_cfg = self.config_parser.raw_config_for(prefix)
        if raw_cfg is not None and raw_cfg.get("format") == "fp8":
            from .schemes.inc_fp8 import INCFp8Linear

            if not isinstance(layer, LinearBase):
                raise NotImplementedError(
                    f"inc fp8 format only supports Linear layers, got "
                    f"{type(layer).__name__} for {prefix}"
                )
            return INCFp8Linear()
