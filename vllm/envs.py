...
if TYPE_CHECKING:
    ...
    VLLM_DFLASH_C1_TAIL_WINDOW: bool = False
...
environment_variables: dict[str, Callable[[], Any]] = {
    ...
    "VLLM_USE_FLASHINFER_SAMPLER": lambda: (
        ...
        # Windows has no flashinfer wheel; the unconditional `import
        # flashinfer` in vllm.v1.attention.backends.flashinfer (triggered
        # by the default-True path) raises ModuleNotFoundError. Default
        # to False on Windows so the Triton fallback is used instead.
    ...
    # DFlash C1: defer draft-KV writes during chunked prefill. Draft context
    # K/V is only computed/written for the trailing sliding window once the
    # request's prefill completes, instead of writing every chunk (whose rows
    # are evicted by the sliding window before decode ever reads them).
    "VLLM_DFLASH_C1_TAIL_WINDOW": lambda: bool(
        int(os.getenv("VLLM_DFLASH_C1_TAIL_WINDOW", "0"))
    ),
